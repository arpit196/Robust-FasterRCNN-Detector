...
def evaluate(model, eval_data = None, num_samples = None, plot = False, print_average_precisions = False, noise=False):
  if eval_data is None:
    if noise:
      eval_data = voc.Dataset(dir = options.dataset_dir, split = options.eval_split, augment = False, shuffle = False,noise='contrast')
    else:  
...
def train(model):
  ...
  training_data = voc.Dataset(dir = options.dataset_dir, split = options.train_split, augment = not options.no_augment, shuffle = True, cache = False) #options.cache_images
  ...
  for epoch in range(1, 1 + options.epochs):
    ...
    checkpoint_file = os.path.join(options.checkpoint_dir, "checkpoint-epoch-bn-test.h5")
    ...
    if options.checkpoint_dir:
      checkpoint_file = os.path.join(options.checkpoint_dir, "checkpoint-bn3-epoch-%d-mAP-%1.1f.h5" % (epoch, mean_average_precision))
      model.save_weights(filepath = checkpoint_file, overwrite = True, save_format = "h5")
...
def detect(model, frame, original_frame, score_threshold=0.7, show_image=True):
    # Preprocess the input frame to model input format (e.g., normalized, resized)
    image_data = _preprocess_vgg16(np.array(frame).astype('float32'))  # Your function that prepares model input from raw frame

    # Anchor generation
    anchor_map, anchor_valid_map = anchors.generate_anchor_maps(image_shape=image_data.shape, feature_pixels=16)
    anchor_map = np.expand_dims(anchor_map, axis=0)                 # batch size 1
    anchor_valid_map = np.expand_dims(anchor_valid_map, axis=0)     # batch size 1
    image_data = np.expand_dims(image_data, axis=0)                 # batch size 1

    x = [image_data, anchor_map, anchor_valid_map]

    # Run prediction
    scored_boxes_by_class_index = model.predict_on_batch(x=x, score_threshold=score_threshold)
    ...
    for class_index, boxes_scored in scored_boxes_by_class_index.items():
        ...
        for box in boxes_scored:
            ...
            left, top = max(0, min(width-1, int(xmin))), max(0, min(height-1, int(ymin)))
            right, bottom = max(0, min(width-1, int(xmax))), max(0, min(height-1, int(ymax)))
            print(f'{top} {left} {bottom} {right}')
            cv2.rectangle(original_frame, (left, top), (right, bottom), (0, 0, 255), 5)
            label = f"{class_name}"
            #: {score:.2f}
            cv2.putText(original_frame, label, (left, max(top-10, 0)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            break

    #if show_image:
    #    cv2.imshow("Realtime Faster R-CNN Detection", frame)
    
    # Show the frame with detections
    cv2.imshow("Realtime Faster R-CNN Detection", original_frame)
    

    return original_frame #scored_boxes_by_class_index
...
def render(model):
    ...
    while True:
        ...
        orig_frame = detect(model,frame=rgb_frame,original_frame=frame,show_image=True)

        # Draw detection boxes and labels on the original frame (BGR)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--deploy", action = "store_true", help = "Run detector on camera")
  ...
  if options.load_from:
    ...
    #print('Nweights')
    #print(np.mean(model._stage1_feature_extractor._block1_in1.nweight))
  else:
  ...
  if options.train:
    ...
    evaluate(model = model, plot = options.plot, print_average_precisions = True,noise=True)
